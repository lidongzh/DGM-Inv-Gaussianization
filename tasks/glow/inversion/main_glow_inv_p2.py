...
# using the second parameterization: extract patches from multi-scale maps
from dgminv.networks.glow_wrapper2 import GlowWrap
...
with torch.no_grad():
    if sensor_type == 'GaussianCS':
        ...
        # fwd_op_w = GaussianSmoothing(opt['in_channel'], 3.0).to(device)
        # th_obs_data = add_noise(fwd_op_w(th_img_true), sigma=noise_std, mode='gaussian', seed=1234)
...
n_bins = 2 ** opt['n_bits']
glow_inv = Glow(opt['in_channel'], opt['n_flow'], opt['n_block'], \
...
if opt['beta'] == 0.0:
    ...
    class InvSolver2(InvSolver):
        def __init__(self, th_obs_data, fwd_op, model_inv, lpips_calc, psnr_calc, \
            ssim_calc, range_convert_func=range_convert_func, plot_img_func=None, \
                result_dir_name='', save_binary=False, is_glow=False, beta=0.0):

            super().__init__(th_obs_data, fwd_op, model_inv, lpips_calc, psnr_calc, \
                ssim_calc, range_convert_func=range_convert_func, plot_img_func=plot_img_func, \
                    result_dir_name=result_dir_name, save_binary=save_binary, is_glow=is_glow)

            self.beta = beta
            self.n_bins = 2 ** model_inv.n_bits
            self.n_pixel = model_inv.nc * model_inv.nh * model_inv.nw

        def compLoss(self):
            th_curr_img, logdet, logP  = self.model_inv()
            
            th_curr_img = self.range_convert_func(th_curr_img)
            th_calc_data = self.fwd_op(th_curr_img)
            
            loss1 = self.lossFunc(th_calc_data, self.th_obs_data)
            b_size = logP.size(0)
            # loss2 = self.beta * (-logP - logdet + np.log(self.n_bins)*self.n_pixel).mean()/(self.n_pixel*np.log(2.0))
            loss2 = self.beta * (-logP - logdet).mean()/(np.log(2.0))
            print('loss1 = ', loss1)
            print('loss2 = ', loss2)
            loss = loss1 + loss2
            ...
            if self._iter == self._loc_counter + 1:
                ...
                with open(self.result_dir_name + '/losses.txt', 'a') as text_file:
                    text_file.write("%d %s %s %s\n" % (self._iter, loss.item(), loss1.item(), loss2.item()))
                self._loc_counter += 1
            return loss
    inv_solver = InvSolver2(th_obs_data, fwd_op, model_inv, lpips_calc, psnr_calc, ssim_calc, \
        range_convert_func=range_convert_func, plot_img_func=plot_img_func, result_dir_name=result_dir_name, \
            save_binary=opt['save_binary'], is_glow=True, beta=opt['beta'])
